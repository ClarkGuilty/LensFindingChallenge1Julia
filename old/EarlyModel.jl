...
lf1 = Dense(3200,1024)                          #512  -> 409600, 410112
lfd1 = Flux.Dropout(0.5,dims=1)
lfb1 = Flux.BatchNorm(1024,affine=true)        #512-> 1024
lf2 = Dense(1024,2)                            #512 -> 1026
l5 = Chain(lf0,lf1)
...
if first_execution
  ...
  @load "earlyModel_decayingRate.bson" weights opt history_loss history_accuracy iterations#Comment if first execution.
...
function alternative_train!(trainData::ImageDataset, testData, nbatches, nepochs, loss , opt, parameters,accuracy, history_loss, history_accuracy, iterations)
  ...
  for i in 1:nbatches
    ...
    for j in 1:nepochs
      #opt.eta = ParameterSchedulers.next!(s)
      if Int((i-1)*nepochs+j+i_base) % 150 == 0
        testmode!(f,true)
        t_loss, t_accuracy, _ = loss_and_accuracy((testData[1],testData[2]),400)
        push!(history_accuracy, t_accuracy)
        push!(history_loss, t_loss)
...
function long_train(trainData::ImageDataset, testData, nbatches,
 ...
 history_loss, history_accuracy, iterations, sleep_time::Int)
 alternative_train!(trainData, testData, nbatches, nepochs,loss,
  ...
  @show opt.eta = opt.eta * 0.99
end
##
#@epochs 15 long_train(trainData, (test_images, test_labels), 80, 12,loss_f,
#  opt,parameters,accuracy,history_loss, history_accuracy,iterations,120)
...
@save "earlyModel_decayingRate.bson" weights opt history_loss history_accuracy iterations
#exit()
...
# p0 = [-1,0.1,1]
# fit = LsqFit.curve_fit(model,x[1:end],y[1:end],p0)
# y0b = model(iterations,p1)
# plot(iterations,[y0b, y],label = nothing)
#fit
