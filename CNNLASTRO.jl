...
#Using (almost) LASTRO architecture.
##
...
lf1 = Dense(3200,1024,mrelu)                          #1024  -> 3276800, 3277824
lfd1 = Flux.Dropout(0.5,dims=1)
lf2 = Dense(1024,1024,mrelu)                          #1024 -> 1048576, 1049600
lfd2 = Flux.Dropout(0.5,dims=1)
lf3 = Dense(1024,1024,mrelu)                          #1024 -> 1048576, 1049600
lfb1 = Flux.BatchNorm(1024,affine=true)         #1024-> 2048
lf4 = Dense(1024,2)                             #2048 -> 2050
l5 = Chain(lf0,lf1)
...
if first_execution
  ...
  history_accuracy = []
  iterations = []
  scheduler = ParameterSchedulers.Stateful((ParameterSchedulers.Inv(λ = 1e-3, γ = 0.05, p = 11)))
else
...
function alternative_train!(trainData::ImageDataset, testData, nbatches, nepochs, loss , opt, parameters,accuracy, history_loss, history_accuracy, iterations)
  ...
  for i in 1:nbatches
    ...
    for j in 1:nepochs
      ...
      if Int((i-1)*nepochs+j+i_base) % 250 == 0
        testmode!(f,true)
        t_loss, t_accuracy, _ = loss_and_accuracy((testData[1],testData[2]),400)
        push!(history_accuracy, t_accuracy)
        push!(history_loss, t_loss)
...
@epochs 14 long_train(trainData, (test_images, test_labels), 160, 12,loss_f,
